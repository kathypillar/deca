// history.c
// modified from history.c from nano
...
bool have_statedir(void)
{
	...
	if (homedir != NULL) {
		statedir = concatenate(homedir, "/.deca/");
	...
	if (xdgdatadir != NULL)
		statedir = concatenate(xdgdatadir, "/deca/");
	else
		statedir = concatenate(homedir, "/.local/share/deca/");
	...
	if (stat(statedir, &dirinfo) == -1) {
		...
		jot_error(N_("Path %s is not a directory and needs to be.\n"
								"deca will be unable to load or save "
								"search history or cursor positions.\n"),
								statedir);
		return FALSE;
	}
