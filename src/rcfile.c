// rcfile.c
// modified from rcfile.c from nano
...
#ifndef RCFILE_NAME
#define HOME_RC_NAME  ".decarc"
#define RCFILE_NAME  "decarc"
#else
...
void jot_error(const char *msg, ...)
{
	...
	if (startup_problem == NULL) {
		if (decarc != NULL) {
			snprintf(textbuf, MAXSIZE, _("Mistakes in '%s'"), decarc);
...
keystruct *strtosc(const char *input)
{
	...
	if (!strcmp(input, "cancel"))
		...
		s->func = do_down;
#if !defined(DECA_TINY) || defined(ENABLE_HELP)
...
void begin_new_syntax(char *ptr)
{
	...
	live_syntax->filename = copy_of(decarc);
...
void grab_and_store(const char *kind, char *ptr, regexlisttype **storage)
{
	...
	while (*ptr != '\0') {
		...
		if (!compile(regexstring, DECA_REG_EXTENDED | REG_NOSUB, &packed_rgx))
			continue;
...
bool parse_syntax_commands(char *keyword, char *ptr)
{
	if (strcmp(keyword, "color") == 0)
		parse_rule(ptr, DECA_REG_EXTENDED);
	else if (strcmp(keyword, "icolor") == 0)
		parse_rule(ptr, DECA_REG_EXTENDED | REG_ICASE);
		...
		for (funcstruct *f = allfuncs; f != NULL; f = f->next) {
			if (f->func == vitals[v] && (f->menus & inmenus[v])) {
				if (first_sc_for(inmenus[v], f->func) == NULL) {
					...
					die(_("If needed, use deca with the -I option "
								"to adjust your decarc settings.\n"));
				} else
					break;
			}
		}
	...
	while ((length = getline(&buffer, &size, rcstream)) > 0) {
		...
		if (!just_syntax && strcmp(keyword, "extendsyntax") == 0) {
			...
			if (strcmp(keyword, "header") == 0 || strcmp(keyword, "magic") == 0) {
				...
				newitem->filename = copy_of(decarc);
...
#endif
		} else if (intros_only && just_syntax && (strcmp(keyword, "set") == 0 ||
		...
		} else if (strcmp(option, "scopes") == 0) {
			if (has_blank_char(argument))
				jot_error(N_("Non-blank characters required"));
			else
				scopes = mallocstrcpy(scopes, argument);
...
/* Read and interpret one of the two decarc files. */
void parse_one_decarc(void)
{
	FILE *rcstream = fopen(decarc, "rb");
	...
	if (rcstream != NULL)
		...
		jot_error(N_("Error reading %s: %s"), decarc, strerror(errno));
}

bool have_decarc(const char *path, const char *name)
{
	if (path == NULL)
		return FALSE;

	free(decarc);
	decarc = concatenate(path, name);

	return is_good_file(decarc);
}

/* Process the decarc file that was specified on the command line (if any),
 * and otherwise the system-wide rcfile followed by the user's rcfile. */
void do_rcfiles(void)
{
	if (custom_decarc) {
		decarc = get_full_path(custom_decarc);
		if (decarc == NULL || access(decarc, F_OK) != 0)
			die(_("Specified rcfile does not exist\n"));
	} else
		decarc = mallocstrcpy(decarc, SYSCONFDIR "/decarc");

	if (is_good_file(decarc))
		parse_one_decarc();

	if (custom_decarc == NULL) {
		const char *xdgconfdir = getenv("XDG_CONFIG_HOME");

		get_homedir();

		/* Now try to find a decarc file in the user's home directory or in the
		 * XDG configuration directories, and process the first one found. */
		if (have_decarc(homedir, "/" HOME_RC_NAME) ||
					have_decarc(xdgconfdir, "/deca/" RCFILE_NAME) ||
					have_decarc(homedir, "/.config/deca/" RCFILE_NAME))
			parse_one_decarc();
