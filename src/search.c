// search.c
// modified from search.c from nano
...
bool regexp_init(const char *regexp)
{
	int value = regcomp(&search_regexp, regexp,
				DECA_REG_EXTENDED | (ISSET(CASE_SENSITIVE) ? 0 : REG_ICASE));
...
{
#ifdef ENABLE_HISTORIES
	/* If nothing was searched for yet during this run of deca, but
...
void do_find_bracket(void)
{
	...
	if (ch == NULL) {
		ch = mbstrchr(matchbrackets, openfile->current->data + openfile->current_x+1);
			if (ch == NULL || tailmatched) {
				ch = mbstrchr(matchbrackets, openfile->current->data + openfile->current_x-1);
					if (ch == NULL || headmatched) {
						refresh_needed=TRUE;
						headmatched=FALSE;
						tailmatched=FALSE;
						return;
		...
		if (balance == 0) {
			if(!current_is_offscreen()) {
				...
				place_the_cursor();
		        	...
		        	waddnstr(midwin,bracket_pair+1,1);
		        	wattroff(midwin, interface_color_pair[BRACKETS]);
			}
			openfile->current = was_current;
			openfile->current_x = was_current_x+offset;
			place_the_cursor();
		        wattron(midwin, interface_color_pair[BRACKETS]);
		        waddnstr(midwin,bracket_pair,1);
		        wattroff(midwin, interface_color_pair[BRACKETS]);
			openfile->current_x = was_current_x;
			refresh_needed=TRUE;
			return;
