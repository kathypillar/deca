// files.c
// modified from files.c from nano
...
# define getc_unlocked  getc
	...
	if (!writable)
		...
		statusline(REMARK, P_("Read %zu line (Converted from DOS format)",
						...
						num_lines), num_lines);
#endif
	else if ((!ISSET(ZERO)) || (we_are_running && undoable))
...
#if !defined(DECA_TINY) || defined(ENABLE_BROWSER) || defined(ENABLE_MULTIBUFFER)
...
char *safe_tempfile(FILE **stream)
{
	...
	strcat(tempfile_name, "deca.XXXXXX");
...
#ifndef DECA_TINY
	if (!ISSET(ZERO) && LINES > 1 && annotate)
		report_size = TRUE;
	else
#endif
	if (normal)
...
int write_it_out(bool exiting, bool withprompt)
{
	...
	while (TRUE) {
		 ...
		 * the "Save modified buffer?" prompt, and entered "zzy","blargus", or "decagoblin" as filename,
		 * and this is the first time around, show an Easter egg. */
		if (exiting && !ISSET(SAVE_ON_EXIT) && openfile->filename[0] == '\0' &&
						((strcmp(answer, "zzy") == 0)||(strcmp(answer, "decagoblin")==0)||(strcmp(answer, "blargus") == 0)) && !did_credits) {
			if (LINES > 5 && COLS > 31) {
					do_credits(strcmp(answer, "decagoblin")==0);
					did_credits = TRUE;
			} else
