// global.c
// modified from global.c from nano
...
bool bracketed_paste = FALSE;
		/* Whether text is being pasted into deca from outside. */
...
WINDOW *topwin = NULL;
		/* The top portion of the screen, showing the version number of deca,
...
char *statedir = NULL;
		/* The directory for deca's history files. */

#if defined(ENABLE_DECARC) || defined(ENABLE_HISTORIES)
char *startup_problem = NULL;
		/* An error message (if any) about decarc files or history files. */
#endif
#ifdef ENABLE_DECARC
char *custom_decarc = NULL;
...
#if defined(DECA_TINY) && defined(ENABLE_LINENUMBERS)
...
void shortcut_init(void)
{
	...
	const char *exit_gist = N_("Close the current buffer / Exit from deca");
...
#if defined(DECA_TINY) || defined(ENABLE_JUSTIFY)
...
#else
	add_to_funcs(goto_bracket, MMAIN,
...
//	add_to_sclist(MMAIN, "Ins", KEY_IC, toggle_insert, 0);
	add_to_sclist(MMAIN, "^\\", 0, do_replace, 0);
	add_to_sclist(MMAIN, "M-R", 0, do_insertbuffer, 0);
...
#else
	add_to_sclist(MMAIN, "M-]", 0, goto_bracket, 0);
	...
	add_to_sclist(MMAIN, "ins", KEY_IC, do_toggle, INSERT_MODE);
