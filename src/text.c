// text.c
// modified from text.c from nano
...
void do_enter(void)
{
...
#endif /* DECA_TINY */
	newnode->data = nmalloc(strlen(openfile->current->data + openfile->current_x) + extra + 1+strlen("\t"));
	...
	if (ISSET(AUTOINDENT)) {
		...
		//if there is a scope delinator set than this well add an extra indent as needed
			if(scopes!=NULL) {
				for(int i=0;i<strlen(scopes);i++) {
					if(openfile->current->data[openfile->current_x-1]==scopes[i]) {
						extra+=strlen("\t");
						strcat(newnode->data,"\t");
					}
				}
...
void treat(char *tempfile_name, char *theprogram, bool spelling)
{
	...
	if ((thepid = fork()) == 0) {
		 ...
		 * so deca doesn't get pushed past the wait(). */
		block_sigwinch(TRUE);
		wait(&program_status);
