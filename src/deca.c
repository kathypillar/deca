// deca.c
// modified from nano.c from nano
...
/* Close the current buffer, and terminate deca if it is the only buffer. */
...
/* Save the current buffer under the given name (or "deca.<pid>" when nameless)
...
void emergency_save(const char *filename)
{
	...
	if (*filename == '\0') {
		...
		sprintf(plainname, "deca.%u", getpid());
	} else
	...
	if (*targetname == '\0')
		...
		fprintf(stderr, _("\nBuffer written to %s\n"), targetname);
#if !defined(DECA_TINY) && defined(HAVE_CHMOD) && defined(HAVE_CHOWN)
...
/* Initialize the three window portions deca uses. */
...
/* Explain how to properly use deca and its command-line options. */
void usage(void)
{
	printf(_("Usage: deca [OPTIONS] [[+LINE[,COLUMN]] FILE]...\n\n"));
	/* TRANSLATORS: The next two strings are part of the --help output.
	 * It's best to keep its lines within 80 characters. */
	printf(_("To place the cursor on a specific line of a file, put the line number with\n"
				"a '+' before the filename.  The column number can be added after a comma.\n"));
	printf(_("When a filename is '-', deca reads data from standard input.\n\n"));
	...
	print_opt("-I", "--ignorercfiles", N_("Don't look at decarc files"));
	...
	print_opt(_("-f <file>"), _("--rcfile=<file>"),
					N_("Use only this file for configuring deca"));
	...
	print_opt("-q", "--noindicator", N_("Do not show a position+portion indicator"));
#ifdef ENABLED_WRAPORJUSTIFY
...
/* Display the version number of this deca, a copyright notice, some contact
 * information, and the configuration options this deca was compiled with. */
void version(void)
{
	printf(_(" deca, version %s\n"), VERSION);
...
#endif
#ifndef ENABLE_DECARC
	printf(" --disable-decarc");
#endif
...
void signal_init(void)
{
	...
	deed.sa_handler = suspend_deca;
	sigaction(SIGTSTP, &deed, NULL);
#endif
#ifdef SIGCONT
	sigfillset(&deed.sa_mask);
	deed.sa_handler = continue_deca;
	sigaction(SIGCONT, &deed, NULL);
#endif

#if !defined(DECA_TINY) && !defined(DEBUG)
	if (getenv("DECA_NOCATCH") == NULL) {
...
{
	die(_("Sorry! deca crashed!  Code: %d.  Please report a bug.\n"), signal);
}
#endif

#ifndef DECA_TINY
/* Handler for SIGTSTP (suspend). */
void suspend_deca(int signal)
{
#ifdef ENABLE_MOUSE
	disable_mouse_support();
#endif
	...
	printf(_("Use \"fg\" to return to deca.\n"));
	fflush(stdout);
...
/* When permitted, put deca to sleep. */
...
#if !defined(DECA_TINY) || defined(ENABLE_SPELLER) || defined(ENABLE_COLOR)
...
void regenerate_screen(void)
{
	...
	endwin();
	refresh();
	if(!ISSET(NOINDICATOR))
	sidebar = (LINES > 5 && COLS > 9) ? 1 : 0;
...
/*		case CONSTANT_SHOW:
	...
	if (flag == AUTOINDENT || flag == BREAK_LONG_LINES || flag == SOFTWRAP) {
		if (!ISSET(ZERO) && ISSET(STATEFLAGS))
			return;
		if (ISSET(STATEFLAGS))
			titlebar(NULL);
	}
...
void unbound_key(int code)
{
	if (code == FOREIGN_SEQUENCE)
		 ...
		 * (from the keyboard) that deca does not recognize. */
...
void inject(char *burst, size_t count)
{
	...
	if((openfile->current_x < strlen(thisline->data))&&ISSET(INSERT_MODE)) {
		thisline->data = nrealloc(thisline->data, datalen + 1);
		memmove(thisline->data + openfile->current_x,thisline->data + openfile->current_x,openfile->current_x + 1);
...
int main(int argc, char **argv)
{
	...
	bool ignore_rcfiles = FALSE;
		/* Whether to ignore the decarc files. */
...
#endif
			case 'f':
				custom_decarc = mallocstrcpy(custom_decarc, optarg);
				break;
#if defined(ENABLE_BROWSER) || defined(ENABLE_HELP)
			case 'g':
				SET(SHOW_CURSOR);
	...
	if (!ignore_rcfiles) {
		...
		alt_speller = NULL;
#endif
		/* Now process the system's and the user's decarc file, if any. */
		do_rcfiles();
	...
	quoterc = regcomp(&quotereg, quotestr, DECA_REG_EXTENDED);
	if (quoterc != 0) {
...
#define NOTREBOUND  TRUE
	...
	if (*openfile->filename == '\0' && openfile->totsize == 0 &&
		...
		statusbar(_("Welcome to deca.  For basic help, type Ctrl+G."));
#endif
