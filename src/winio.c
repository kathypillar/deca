// winio.c
// modified from winio.c from nano
...
{
//	blank_row(footwin, 0);
	wattron(footwin, interface_color_pair[TITLE_BAR]);
	mvwprintw(footwin, 0, 0, "%*s", COLS, " ");
	wattroff(footwin, interface_color_pair[TITLE_BAR]);
}
...
void wipe_statusbar(void)
{
	...
	if ((ISSET(ZERO) || LINES == 1) && currmenu == MMAIN)
		return;
	blank_statusbar();
//	blank_row(footwin, 0);
...
int buffer_number(openfilestruct *buffer, int* length)
{
	int count = 1;

	while (buffer != startfile) {
		*length = *length + strlen(buffer->filename)+4;
		buffer = buffer->prev;
		count++;
	}
		*length = *length + strlen(buffer->filename)+4;
 ...
 * version of deca, the current filename, and whether the current file
...
void titlebar(const char *path)
{
	...
	as_an_at = FALSE;
		...
		if (more_than_one) {
			ranking = nmalloc(24);
			curNum = buffer_number(openfile,&curLen);
			max = buffer_number(startfile->prev,&maxLen);
			sprintf(ranking, "[%s%i/%i]","  "+((max<10)||(curNum>=10))+((max<100)||(curNum>=100)), curNum,max); //if you are opening a four digit number of tabs you are a menace what is wrong with you.
			upperleft = ranking;
		} else
#endif
			upperleft = "";
	...
	wmove(topwin, 0, verlen);
			openfilestruct *buffer=startfile;
			bool b=1;
			if(prevLen>curLen&&prevLen-curLen<COLS-20)//this back scrolling could be better but I don't care at this point
				curLen=prevLen;
			prevLen=curLen;
			while(curLen+verlen>COLS) { //7 is the total space used by the interface thats not file names
				curLen=curLen-(strlen(buffer->filename)+4);
				buffer=buffer->next;
			}
			for(int i=0;i<max&&b;i++) {
				if(i==0&&buffer!=startfile) {
					if(buffer->prev==openfile) {
						buffer=buffer->prev;
					}
					if(buffer!=startfile)
						waddstr(topwin,"<- ");
				...
				if(buffer==openfile) {
					...
					wattron(topwin, interface_color_pair[SPOTLIGHTED]);
				}
				waddstr(topwin,"| ");
				if (buffer->filename[0] == '\0')
					waddstr(topwin, "New Buffer");
				else
					waddstr(topwin, buffer->filename);
				...
				if(buffer==openfile) {
					wattron(topwin, interface_color_pair[TITLE_BAR]);
					wattroff(topwin, interface_color_pair[SPOTLIGHTED]);
				}
				if((buffer->next==startfile&&buffer!=startfile))
					b=0;
				buffer=buffer->next;
				if(b&&(getcurx(topwin)+(strlen(buffer->filename)+4)+4>COLS)) {
					waddstr(topwin," -> ");
					b=0;
				}
			}
...
void minibar(void)
{
	...
	mvwaddstr(footwin, 0, 2, BRANDING);


	sprintf(location,"[%zi,%zi]",openfile->current_x,openfile->current->lineno);
	// Display the line/column position of the cursor.
	if (placewidth + 32 < COLS)
		mvwaddstr(footwin, 0, COLS - 7 - placewidth, location);
	...
	if (ISSET(STATEFLAGS) && !successor && 14 + 2 * padding < COLS) {
...
#if defined(ENABLE_MULTIBUFFER) && !defined(DECA_TINY)
...
void bottombars(int menu)
{
	...
	if (ISSET(NO_HELP) || LINES < (ISSET(ZERO) ? 3 : 5))
		return;
...
void draw_row(int row, const char *converted, linestruct *line, size_t from_col)
{
	...
	if (margin > 0) {
			if(openfile->current->lineno-openfile->edittop->lineno==row)
			wattron(midwin, interface_color_pair[BRACKETS]);
			else
		...
		wattroff(midwin, interface_color_pair[BRACKETS]);
...
void edit_redraw(linestruct *old_current, update_type manner)
{
	...
	if (line_needs_update(was_pww, openfile->placewewant) || (old_current != openfile->current && get_page_start(openfile->placewewant) > 0))
		update_line(openfile->current, openfile->current_x);
	do_find_bracket();
}
...
void do_credits(bool dance)
{
	...
	const char *credits[CREDIT_LEN] = {
		...
		"Albert Chin",
		"",
		NULL,                /* "Special thanks to from nano:" */
	...
	const char *xlcredits[XLCREDIT_LEN] = {
		...
		N_(" | |__  |()| | "),
		N_("  \\___| \\_/|_| "),
		N_("The deca text editor"),
		N_("version"),
		N_("Brought to you by:"),
		N_("Special thanks to:"),
		N_("For PCRE:"),
		N_("Based on nano from:"),
		N_("Special thanks from nano:"),
		N_("The Free Software Foundation"),
		N_("the many translators and the TP"),
		N_("For ncurses:"),
		N_("Thank you for using deca!"),
		N_(" \\___/  "),
		N_(" /. .\\/ "),
		N_("/\\___/  "),
		N_("  | |   "),
		N_("-- ars longa, vita brevis --")
	};
	...
	for (crpos = 0; crpos < (CREDIT_LEN) + editwinrows / 2; crpos++) {
		if (crpos < (CREDIT_LEN)+ ((editwinrows/2)*dance)) {
			for(int j = 0; j < 4;j++) {
				const char *text = credits[(crpos+j)%(16+(1000*!dance))];

				if (!text)
					text = _(xlcredits[xlpos++]);

				mvwaddstr(midwin, editwinrows -1-(9*dance)+j, (COLS - breadth(text)) /2, text);
				wrefresh(midwin);
				if(!dance)
					break;
			}
		...
		if (wgetch(midwin) != ERR)
			break;
		if(!dance) {
			if (crpos <CREDIT_LEN-6 && crpos > 8) { //in order to make the ascii art work
				napms(600);
				wscrl(midwin, 1);
				wrefresh(midwin);
			}
